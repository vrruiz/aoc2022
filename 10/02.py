...
class Cpu():
    ...
    def ejecutar(self):
        seguir = True
        while seguir == True:
            crt_linea = self.ciclo // 40
            crt_pos = self.ciclo % 40
            self.ciclo += 1
            if self.ciclo <= 240:
                pixel = '.'
                if self.registros['x'] in [crt_pos-1,crt_pos,crt_pos+1]:
                    pixel = '#'
                self.crt[crt_linea] = self.crt[crt_linea] + pixel
                # print(self.ciclo, crt_linea, crt_pos, self.registros['x'], pixel)
